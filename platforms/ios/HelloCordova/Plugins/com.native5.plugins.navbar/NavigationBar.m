...
-(void) create:(CDVInvokedUrlCommand*)command
{
    ...
    if(options)
    {
        ...
        {
//            navBar.backgroundColor = [UIColor colorWithRed:218.0f green:37.0f blue:29.0f alpha:1.0f];
            NSString *imageURL = [options objectForKey:@"background"];
...
//            [[self.navBarController.appearance] setTranslucent:NO];
        }
    }
...
//[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:imageName] style:UIBarButtonItemStylePlain target:self action:actionOnSelf];
            UIBarButtonItem *negativeSeparator = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace target:self action:actionOnSelf];
            negativeSeparator.image =[UIImage imageNamed:imageName];
            negativeSeparator.width = -24;
...
- (void)setupLeftButton:(CDVInvokedUrlCommand*)command
{
    ...
    space.width = -10;
    NSMutableArray *btns = [[NSMutableArray alloc] initWithObjects:newButton, nil];
    NSMutableArray *btnArr = [[NSMutableArray alloc] initWithCapacity:btns.count+1];
    [btnArr addObject:space];
    [btnArr addObjectsFromArray:btns];
    
    navBarController.navItem.leftBarButtonItems = btnArr;
//    [newButton release];
}
...
- (void)setupRightButtons:(CDVInvokedUrlCommand*)command
{
    NSString * title1 = [command.arguments objectAtIndex:0];
    NSString * imageName1 = [command.arguments objectAtIndex:1];
    NSString * title2 = [command.arguments objectAtIndex:2];
    NSString * imageName2 = [command.arguments objectAtIndex:3];
    const NSDictionary *options = [command.arguments objectAtIndex:4];
    
    UIBarButtonItem *newButton1 = [self makeButtonWithOptions:options title:title1 imageName:imageName1 actionOnSelf:@selector(rightButton1Tapped)];
    UIBarButtonItem *newButton2 = [self makeButtonWithOptions:options title:title2 imageName:imageName2 actionOnSelf:@selector(rightButton2Tapped)];
    UIBarButtonItem *space = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace target:nil action:nil];
    space.width = -5;
    NSMutableArray *btns = [[NSMutableArray alloc] initWithObjects:newButton1,newButton2, nil];
    NSMutableArray *btnArr = [[NSMutableArray alloc] initWithCapacity:btns.count+1];
    [btnArr addObject:space];
    [btnArr addObjectsFromArray:btns];
    
    navBarController.navItem.rightBarButtonItems = btnArr;
    //    [newButton release];
}
...
{
    NSString * jsCallBack = [NSString stringWithFormat:@"window.plugins.navBar.leftButtonTapped();"];
    [self writeJavascript:jsCallBack];
}

-(void) rightButtonTapped
{
    NSString * jsCallBack = [NSString stringWithFormat:@"window.plugins.navBar.rightButtonTapped();"];
    [self writeJavascript:jsCallBack];
}
-(void) rightButton1Tapped
{
    NSString * jsCallBack = [NSString stringWithFormat:@"window.plugins.navBar.rightButton1Tapped();"];
    [self writeJavascript:jsCallBack];
}
-(void) rightButton2Tapped
{
    NSString * jsCallBack = [NSString stringWithFormat:@"window.plugins.navBar.rightButton2Tapped();"];
    [self writeJavascript:jsCallBack];
}
